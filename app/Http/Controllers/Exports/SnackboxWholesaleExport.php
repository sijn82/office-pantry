...
class SnackboxWholesaleExport implements WithMultipleSheets
{
        ...
        switch ($this->delivery_days) {
            case 'mon-tue':
                foreach ($mon_tues as $route_day) {
                    $sheets[] = new SnackboxWholesaleSingleCompanyExportNew($route_day, $this->week_start);
                }
                return $sheets;
                break;
            case 'wed-thur-fri':
                ...
                break;
            case 'mon':
                $sheets[] = new SnackboxWholesaleSingleCompanyExportNew('Monday', $this->week_start);
                return $sheets;
                break;
            case 'tue':
                $sheets[] = new SnackboxWholesaleSingleCompanyExportNew('Tuesday', $this->week_start);
                return $sheets;
                break;
            case 'wed':
                $sheets[] = new SnackboxWholesaleSingleCompanyExportNew('Wednesday', $this->week_start);
                return $sheets;
                break;
            case 'thur':
                $sheets[] = new SnackboxWholesaleSingleCompanyExportNew('Thursday', $this->week_start);
                return $sheets;
                break;
            case 'fri':
                $sheets[] = new SnackboxWholesaleSingleCompanyExportNew('Friday', $this->week_start);
                return $sheets;
                break;
        }
    }
  ...
  implements FromView, WithEvents, ShouldAutoSize, WithTitle //, WithMultipleSheets
{
    ...
    public function view(): View
   {
        ...
        $snackboxes = SnackBox::where('delivered_by', $this->courier)->where('next_delivery_week', $this->week_start)
                                ->where('product_id', '!=', 0)->where('type', '=', 'wholesale')->where('delivery_day', $this->day)->get();
        //dd($snackboxes);
    ...
    {
       return 'OP Wholesale Snacks - ' . $this->day;
    }
