...
class WeeklyInvoicesExportV2 implements
FromView,
ShouldAutoSize,
WithTitle
{
    ...
    // This can now process OP and Fruitpartner punnets (current and archived).
    public function calculatePunnets($fruitbox, &$punnets_weekly_total, &$punnets_weekly_total_fruit_partner)
    {
        if ( strtolower( $fruitbox->type ) === 'seasonal' ) {
            ...
            if ($fruitbox->fruit_partner_id === 1) {
                ...
                return $punnets_weekly_total;

            } else {

                $punnets_weekly_total_fruit_partner += ( $fruitbox->fruitbox_total * 2 );
                return $punnets_weekly_total_fruit_partner;
            }
    ...
    // Input Collection $fruitbox // Output Object(s) $monday/$tuesday/$wednesday/$thursday/$friday_total_boxes or $friday_total_boxes_fruit_partner
    public function combineFruitBoxOrdersByDayAndType(
        $fruitbox,
        &$monday_total_boxes,
        &$tuesday_total_boxes,
        &$wednesday_total_boxes,
        &$thursday_total_boxes,
        &$friday_total_boxes,
        &$monday_total_boxes_fruit_partner,
        &$tuesday_total_boxes_fruit_partner,
        &$wednesday_total_boxes_fruit_partner,
        &$thursday_total_boxes_fruit_partner,
        &$friday_total_boxes_fruit_partner
        )
    {
        // I know this is still too much duplication but putting the check here is more readable than inserting the check on each day, then returning the correct variable.

        // This checks for Office Pantry deliveries
        if ($fruitbox->fruit_partner_id === 1) {
            ...
            // It must be a Fruit Partner Delivery.
            switch ($fruitbox->delivery_day) {
                case 'Monday':
                    // I want the fruitbox types to be as dynamic as possible, so we can create new ones without worrying about hardcoded checks.
                    // This isset checks to see if the object contains the box type as a property, adding to the totals if it does.
                    if ( isset( $monday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} )) {
                        ...
                        return $monday_total_boxes_fruit_partner;
                    } else {
                        // Otherwise this is the first box of its type to be processed for this company, on this day.
                        // Instead of adding to the total, we can set the inital value.
                        $monday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} = $fruitbox->fruitbox_total;
                ...
                case 'Tuesday':
                    if( isset( $tuesday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} )) {
                        ...
                        return $tuesday_total_boxes_fruit_partner;
                    } else {
                        $tuesday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} = $fruitbox->fruitbox_total;
                        return $tuesday_total_boxes_fruit_partner;
                    }
                ...
                case 'Wednesday':
                    if( isset( $wednesday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} )) {
                        ...
                        return $wednesday_total_boxes_fruit_partner;
                    } else {
                        $wednesday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} = $fruitbox->fruitbox_total;
                        return $wednesday_total_boxes_fruit_partner;
                    }
                ...
                case 'Thursday':
                    if( isset( $thursday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} )) {
                        ...
                        return $thursday_total_boxes_fruit_partner;
                    } else {
                        $thursday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} = $fruitbox->fruitbox_total;
                        return $thursday_total_boxes_fruit_partner;
                    }
                ...
                case 'Friday':
                    if( isset( $friday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} )) {
                        ...
                        return $friday_total_boxes_fruit_partner;
                    } else {
                        $friday_total_boxes_fruit_partner->{strtolower( $fruitbox->type )} = $fruitbox->fruitbox_total;
                        return $friday_total_boxes_fruit_partner;
                    }
    ...
    public function createFruitBoxInvoice(
        $key, $box_total, $company,
        $fruitbox_x1, $fruitbox_x2, $fruitbox_x4, $fruitbox_x7_plus,
        $fruitbox_x1_fruit_partner, $fruitbox_x2_fruit_partner, $fruitbox_x4_fruit_partner, $fruitbox_x7_plus_fruit_partner,
        &$sales_invoice, &$sales_invoices, $fp = 'false'
        )
    {
        if ($key !== 0) {
            ...
            $sales_invoice->email_address = $company->invoice_email;
        ...
        switch ($key) {
            ...
            case 1:
                ...
                $sales_invoice->description = ($fp === 'false') ? $fruitbox_x1->name : $fruitbox_x1_fruit_partner->name;
                $sales_invoice->account_code = ($fp === 'false') ? $fruitbox_x1->sales_nominal : $fruitbox_x1_fruit_partner->sales_nominal;
                $sales_invoice->unit_amount = ($fp === 'false') ? $fruitbox_x1->price : $fruitbox_x1_fruit_partner->price;
                $sales_invoices[] = $sales_invoice;
                break;
            case 2:
                // Currently the only field I need the switch case for, is this the best way?  Maybe give it (improvements) a little think once constructed.
                $sales_invoice->description = ($fp === 'false') ? $fruitbox_x2->name : $fruitbox_x2_fruit_partner->name;
                $sales_invoice->account_code = ($fp === 'false') ? $fruitbox_x2->sales_nominal : $fruitbox_x2_fruit_partner->sales_nominal;
                $sales_invoice->unit_amount = ($fp === 'false') ? $fruitbox_x2->price : $fruitbox_x2_fruit_partner->price;
                $sales_invoices[] = $sales_invoice;
                break;
            case 3:
                // Currently the only field I need the switch case for, is this the best way?  Maybe give it (improvements) a little think once constructed.
                $sales_invoice->description = ($fp === 'false') ? $fruitbox_x3->name : $fruitbox_x3_fruit_partner->name;
                $sales_invoice->account_code = ($fp === 'false') ? $fruitbox_x3->sales_nominal : $fruitbox_x3_fruit_partner->sales_nominal;
                $sales_invoice->unit_amount = ($fp === 'false') ? $fruitbox_x3->price : $fruitbox_x3_fruit_partner->price;
                $sales_invoices[] = $sales_invoice;
                break;
            case 4:
            case 5:
            case 6:
                // Currently the only field I need the switch case for, is this the best way?  Maybe give it (improvements) a little think once constructed.
                $sales_invoice->description = ($fp === 'false') ? $fruitbox_x4->name : $fruitbox_x4_fruit_partner->name;
                $sales_invoice->account_code = ($fp === 'false') ? $fruitbox_x4->sales_nominal : $fruitbox_x4_fruit_partner->sales_nominal;
                $sales_invoice->unit_amount = ($fp === 'false') ? $fruitbox_x4->price : $fruitbox_x4_fruit_partner->price;
                $sales_invoices[] = $sales_invoice;
                break;
            default:
                // Currently the only field I need the switch case for, is this the best way?  Maybe give it (improvements) a little think once constructed.
                $sales_invoice->description = ($fp === 'false') ? $fruitbox_x7_plus->name : $fruitbox_x7_plus_fruit_partner->name;
                $sales_invoice->account_code = ($fp === 'false') ? $fruitbox_x7_plus->sales_nominal : $fruitbox_x7_plus_fruit_partner->sales_nominal;
                $sales_invoice->unit_amount = ($fp === 'false') ? $fruitbox_x7_plus->price : $fruitbox_x7_plus_fruit_partner->price;
                $sales_invoices[] = $sales_invoice;
        }
    }

    public function createTailoringFeeInvoice($tailoring_fee_weekly_total, $company, &$sales_invoice, &$sales_invoices, $fp = 'false')
    {
        ...
        $sales_invoice->po_region = isset($company->invoice_region) ? $company->invoice_region : $company->route_region;
        $sales_invoice->po_post_code = isset($company->invoice_postcode) ? $company->invoice_postcode : $company->route_postcode;
        $sales_invoice->description = ($fp === 'false') ? 'Tailored Fruit Box' : 'Tailored Fruit Box P';
        $sales_invoice->quantity = 1;
        $sales_invoice->account_code = ($fp === 'false') ? '4040' : '4050';
    ...
    public function createPunnetsInvoice($punnets_weekly_total, $company, &$sales_invoice, &$sales_invoices, $fp = 'false')
    {
        $sales_invoice = new $sales_invoice();
        $sales_invoice->invoice_name = $company->invoice_name;
        $sales_invoice->email_address = $company->invoice_email;
        ...
        $sales_invoice->po_region = isset($company->invoice_region) ? $company->invoice_region : $company->route_region;
        $sales_invoice->po_post_code = isset($company->invoice_postcode) ? $company->invoice_postcode : $company->route_postcode;
        $sales_invoice->description = ($fp === 'false') ? 'Seasonal Fruit (Punnet)' : 'Seasonal Fruit (Punnet) P';
        ...
        foreach ($companies as $company) {
            ...
            //---------- End of Unset Some Variables ----------//
                ...
                //----- Drinkbox Variables -----//
                    ...
                    $drinkbox_grand_total = 0; // This is a recent addition (31/05/19) as I hit an undefined variable error
                //----- Drinkbox Variables -----//
            ...
            foreach ($company->fruitbox as $fruitbox) {
                ...
                // We want to separate drops made by us, with drops fulfilled by fruit partners, naming them slightly differently
                if ($fruitbox->fruit_partner_id === 1) {
                    ...
                    $this->calculatePunnets($fruitbox, $punnets_weekly_total, $punnets_weekly_total_fruit_partner);
                    ...
                    if ($fruitbox->discount_multiple === 'Yes') {
                        ...
                        $this->combineFruitBoxOrdersByDayAndType(
                            $fruitbox,
                            $monday_total_boxes, $tuesday_total_boxes, $wednesday_total_boxes, $thursday_total_boxes, $friday_total_boxes,
                            $monday_total_boxes_fruit_partner, $tuesday_total_boxes_fruit_partner, $wednesday_total_boxes_fruit_partner, $thursday_total_boxes_fruit_partner, $friday_total_boxes_fruit_partner
                        );
            ...
            foreach ($company->fruitbox_archive as $fruitbox_archive) {
                ...
                if ($fruitbox_archive->fruit_partner_id === 1) {
                    ...
                    $this->calculatePunnets($fruitbox_archive, $punnets_weekly_total, $punnets_weekly_total_fruit_partner);

                    // EDIT: 08/01/20 - Am I somehow doing this twice?  When did that happen? <- Very confused but my output seems to be accurate now this is commented out?
            ...
            // Fruitboxes grouped by day, minus the ones not applicable for discount (Fruit Partners)
                ...
                foreach ($weekly_box_totals_fruit_partner as $key => $box_total_fruit_partner) {
                    ...
                    $this->createFruitBoxInvoice(
                        $key, $box_total_fruit_partner, $company,
                        $fruitbox_x1, $fruitbox_x2, $fruitbox_x4, $fruitbox_x7_plus,
                        $fruitbox_x1_fruit_partner, $fruitbox_x2_fruit_partner, $fruitbox_x4_fruit_partner, $fruitbox_x7_plus_fruit_partner,
                        $sales_invoice, $sales_invoices, 'true'
                    );
                }

                if ($tailoring_fee_weekly_total > 0) {

                    $this->createTailoringFeeInvoice($tailoring_fee_weekly_total, $company, $sales_invoice, $sales_invoices);
                }

                if ($tailoring_fee_weekly_total_fruit_partner > 0) {

                    $this->createTailoringFeeInvoice($tailoring_fee_weekly_total_fruit_partner, $company, $sales_invoice, $sales_invoices, 'true');
                }


                if ($punnets_weekly_total > 0) {

                    $this->createPunnetsInvoice($punnets_weekly_total, $company, $sales_invoice, $sales_invoices);
                }

                if ($punnets_weekly_total_fruit_partner > 0) {

                    $this->createPunnetsInvoice($punnets_weekly_total_fruit_partner, $company, $sales_invoice, $sales_invoices, 'true');
                }
