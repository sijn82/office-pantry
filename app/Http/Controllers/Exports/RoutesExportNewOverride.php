...
class RoutesExportNewOverride implements WithMultipleSheets
// , WithEvents
{
    // use Exportable;
    ...
    {
        //----- I've removed the old way entirely here but the almost identical Routes version is commented out in RoutesExportNew -----//
        ...
        switch ($this->delivery_days) {
            case 'mon-tue':
                ...
                foreach ($orderedRoutes as $assigned_route) {
                    $sheets[] = new RoutesCollectionNewOverride($assigned_route, $this->week_starting);
                }
                break;
            case 'wed-thur-fri':
    ...
    } // End of sheets() array function


} // End of - class RoutesExport implements WithMultipleSheets

class RoutesCollectionNewOverride implements
FromView,
WithTitle,
WithHeadings,
WithEvents
{
     ...
     public function view(): View
    {
        ...
        foreach ($routeInfoAll as $routeInfoSolo)
        {
            ...
            // Rather than processing the orders like before we just need to pull through the values that are stored directly in the CompanyRoute table.
            // These figures can be entered in the admin panel and allow the admins to override the total determined by the order processing function.
            // This should only be done if the admin wishes to fudge the total number of boxes and drinks as it will not be checking the actual orders on this run through.
