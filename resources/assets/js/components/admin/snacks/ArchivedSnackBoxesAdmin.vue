<template>
    ...
    <div id="archived_snackboxes">
        ...
        <div v-if="Object.keys(this.archived_snackboxes).length"> 

            <archived-snackbox class="archived_snackbox" v-for="(archived_snackbox, key) in this.archived_snackboxes" :key="key" :archived_snackbox="archived_snackbox" :company="this.company" @refresh-data="refreshData($event)"> {{ key }} </archived-snackbox>

        </div>
        <div v-else>
            <ul><li style="list-style:none;">  ~ Nothing To See Here ~ </li></ul>
            <!-- <ul><li style="list-style:none;"> {{ this.archived_snackboxes }} </li></ul> -->
        </div>
    </div>
</template>
